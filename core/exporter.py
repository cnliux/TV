...
class ResultExporter:
    ...
    def _classify_channels(self, channels: List[Channel]) -> tuple:
        ...
        for channel in channels:
            ...
            if "://[" in url:  # 显式的IPv6地址
                ...
                if ":" in domain and not domain.split(":")[0].replace('.', '').isdigit():
                    ip_type = "ipv6"  # 包含冒号但不是IPv4地址
                else:
                    ip_type = Channel.classify_ip_type(url)
            else:
                ip_type = "other"
    ...
    def _export_uncategorized_channels(self, channels: List[Channel]):
        ...
        if uncategorized_channels:
            ...
            logger.info(f"📄 生成的未分类频道文件: {(self.output_dir / uncategorized_path).resolve()}，包含 {count} 个频道")
