...
class AutoCategoryMatcher:
    ...
    def sort_channels_by_template(self, channels: List[Channel], whitelist: Set[str]) -> List[Channel]:
        ...
        for category, channel_names in template_order.items():
            ...
            # 合并白名单和非白名单频道，并按照模板中的顺序统一排序
            all_category_channels = category_channels
            all_category_channels.sort(key=lambda c: self._get_channel_order(c, channel_names))

            # 将排序后的频道添加到结果列表中
            sorted_channels.extend(all_category_channels)
    ...
    def _is_whitelisted(self, channel: Channel, whitelist: Set[str]) -> bool:
        """检查频道是否在白名单中"""
        clean_name = self.normalize_channel_name(channel.name)  # 使用规范化名称进行匹配
        for entry in whitelist:
            if entry in channel.url or channel.url == entry or clean_name == entry:
                return True
        return False
